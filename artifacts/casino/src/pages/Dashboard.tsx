...
export default function Dashboard() {
  ...
  const { data: activity, isLoading: isLoadingActivity } = useGetMyActivity({
    limit: 5,
  });
  ...
  return (
    <AppLayout>
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <PlayerBalance wallet={wallet} isLoading={isLoadingWallet} />

        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Dashboard</h1>
            <p className="text-muted-foreground mt-1">
              Overview of your activity and balances.
            </p>
          </div>
        </div>
          ...
          <Card className="bg-card border-border/50 shadow-md">
            ...
            <CardContent>
              {isLoadingStats ? (
                ...
                <div className="text-2xl font-bold">
                  ${parseFloat(stats?.totalWagers || "0").toFixed(2)}
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="bg-card border-border/50 shadow-md">
            ...
            <CardContent>
              {isLoadingStats ? (
                ...
                >
                  {parseFloat(stats?.netProfit || "0") >= 0 ? "+" : ""}$
                  {parseFloat(stats?.netProfit || "0").toFixed(2)}
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="bg-card border-border/50 shadow-md">
            ...
            <CardContent>
              {isLoadingStats ? (
                ...
                <div className="text-2xl font-bold">
                  {(stats?.winRate ? stats.winRate * 100 : 0).toFixed(1)}%
                </div>
              )}
